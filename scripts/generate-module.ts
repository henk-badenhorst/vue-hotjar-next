...
async function clearBuild() {
  ...
  await fs.rmdir(path.resolve(path.resolve(__dirname, '../'), 'dist'), { recursive: true }).then(() => {
    isCompleted(true)
  }).catch((error) => {
    isCompleted(false)
...
async function createDirStructure() {
  ...
  await fs.mkdir(path.resolve(path.resolve(__dirname, '../'), 'dist/package'), { recursive: true }).then(() => {
    isCompleted(true)
  }).catch((error) => {
    isCompleted(false)
    throw new Error(error)
...
async function runWebpackBuild() {
  ...
  await utils.promisify(child_process.exec)('npx webpack --config webpack.config.js', { encoding: 'utf8' }).then(() => {
    isCompleted(true)
  }).catch((error) => {
    isCompleted(false)
    throw new Error(error)
  ...
  new Promise((resolve, reject) => {
    try {
      files.forEach(async (data) => {
        await fs.copyFile(path.resolve(__dirname, data.source), path.resolve(__dirname, data.destination)).catch((error) => {
          throw new Error(error)
        });
      })
      isCompleted(true)
...
async function updatePackageJson() {
  ...
  const packageJsonPath = path.resolve(path.resolve(__dirname, '../'), 'dist/package/package.json')
  await fs.readFile(packageJsonPath, { encoding: 'utf8' }).then(async (data: any) => {
    const packageJson = {
      ...JSON.parse(data),
      files: ['/'],
      main: 'index.js',
...
async function generatePackage(): Promise<void> {
  ...
  await utils.promisify(child_process.exec)('npm pack ./dist/package/', { encoding: 'utf8' }).then( async (data) => {
    isCompleted(true)
...
async function copyZip(fileName: string): Promise<void> {
  process.stdout.write(chalk.green('7. Copying generating NPM package to dist... '));
  await utils.promisify(child_process.exec)(`cp ./${fileName} ./dist/`, { encoding: 'utf8' }).then(() => {
    isCompleted(true)
  }).catch((error) => {
    isCompleted(false)
    throw new Error(error)
...
async function completionOutput(startTime: number): Promise<void> {
  ...
  process.stdout.write(chalk.green.bold(`Success \t\t\t\t ${chalk.reset.green('(' + duration.toFixed(2) + 'sec)')}`));
}
